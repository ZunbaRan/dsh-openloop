// CRM 模拟数据 + 共享渲染组件（快照内容按 kind+params 渲染）
const LEADS = [
...
function PinIcon({ filled }) {
  return (
    <svg viewBox="0 0 16 16" fill={filled ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.3" aria-hidden="true">
      <path d="M9.6 2.2 13.8 6.4l-1.5.4-2.4 2.4-.3 3-1.5 1.5-2-3.6-3.3 1 .9-.9 2.2-2.2-2.2-3.4 1.6-1.4 3 .3 2.3-2.3z" strokeLinejoin="round" />
    </svg>
  )
}
function CloseIcon() {
...
function LeadTable({ leads, selectedId, onSelect, readonly }) {
  return (
    <table className={`lead-table${readonly ? ' readonly' : ''}`}>
      <thead><tr>
        <th>线索</th><th>联系人</th><th>金额</th><th>阶段</th><th>跟进</th>{!readonly && <th style={{ width: 1 }}></th>}
      </tr></thead>
      <tbody>
        {leads.map(l => (
          <tr key={l.id} className={l.id === selectedId ? 'sel' : ''}
              onClick={() => !readonly && onSelect && onSelect(l)}>
            <td><span className="co"><strong>{l.company}</strong> <span style={{ color: 'var(--dsw-alias-label-caption)', fontSize: 9.5 }}>{l.id}</span></span></td>
            <td>{l.contact}</td>
            <td className="amt">{fmtAmount(l.amount)}</td>
            <td><span className={`stage-pill s${l.stage}`}>{l.stageName}</span></td>
            <td style={{ color: 'var(--dsw-alias-label-caption)', fontSize: 10.5 }}>{l.lastTouch}</td>
            {!readonly && <td><span className="row-link">查看详情 →</span></td>}
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <div className="panel-card">
      <div className="panel-head">
        ...
        <span className="rid">my-crm:contact-card?leadId={l.id}</span>
      </div>
      <div className="detail-grid">
...
// 快照内容渲染器：按 kind + params 还原（= 冻结参数、内容活的语义）
function renderSnapshotContent(snap, onReadonlyClick) {
  if (snap.kind === 'lead-list') {
    return (
      <div onClickCapture={onReadonlyClick}>
        <LeadTable leads={LEADS} readonly selectedId={snap.params.selectedId} />
      </div>
    )
  }
  if (snap.kind === 'lead-detail') return <LeadDetail lead={leadById(snap.params.leadId)} />
  if (snap.kind === 'contact-card') return <ContactCard lead={leadById(snap.params.leadId)} />
  if (snap.kind === 'sales-kpi') return <SalesKpi />
  return <div style={{ padding: 16, fontSize: 11, color: 'var(--dsw-alias-label-caption)' }}>未知快照类型</div>
}

function snapshotTitle(snap) {
  if (snap.kind === 'lead-list') return '线索列表'
  if (snap.kind === 'lead-detail') { const l = leadById(snap.params.leadId); return '线索详情 · ' + (l ? l.company : snap.params.leadId) }
  if (snap.kind === 'contact-card') { const l = leadById(snap.params.leadId); return '联系人卡片 · ' + (l ? l.contact : '') }
  if (snap.kind === 'sales-kpi') return '销售业绩看板'
  return snap.kind
}

function snapshotParamText(snap) {
  const parts = Object.entries(snap.params).map(([k, v]) => `${k}=${v}`)
  return parts.length ? parts.join(' ') : '无参数'
}

Object.assign(window, {
  LEADS, STAGE_NAMES, leadById, fmtAmount,
  SnapIcon, PinIcon, CloseIcon,
  LeadTable, LeadDetail, ContactCard, SalesKpi,
  renderSnapshotContent, snapshotTitle, snapshotParamText,
})
