...
> **状态**：历史草案。方向 1（第三方 MCP Apps 连接路径）仍在开发、尚未完成。禁止按本文正文安装、注册面板或填写 API 钥匙
> **定位**：已废弃的 v1 安装时概念记录，不是给第三方作者的现行说明书
> **设计哲学**（v1 草案当时，已不实施）：包是**安装时**概念，不是**运行时**概念——安装完成后，第三方组件与 agent 现场生成的组件在 registry 里**形态完全相同**，现有渲染/pin/溯源/刷新管线零改动全部复用
