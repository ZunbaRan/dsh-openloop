...
本工具的能力以**已安装的平台包**为限。当你要做的事没有对应 action（如建业务集合、聚合端点、表单预设），这是**平台能力缺口**，不是要绕过的障碍：

1. 输出「缺口报告」给维护者：缺什么 / 证据（调了什么、返回什么）/ 建议的平台扩展项
2. **然后停下等指示**——继续用现有能力能完成的部分照常完成，缺口部分如实标注未实现
3. **禁止的越界方案**：修改 vendor/node_modules 里的平台包、就地打补丁、提议重编译重启用户生产实例（如 3080）——平台包迭代只在 openloop 源仓库进行，走版本化发布纪律（check → bump → pack → 双 profile 安装 → 真机验收），与应用构建会话是两条独立轨道

> 判据：**在平台上建应用 ≠ 改平台**。发现「建不成」时，正确动作是报告缺口，而不是扩展平台。
...
- **平台缺口只报告不代建**：本 skill 的能力清单之外的需求一律走「平台边界」节的报告协议
