...
describe('html node (0.9.0)', () => {
  const htmlDoc = (source: string): unknown => ({
    title: '设计稿',
    layout: 'grid',
    nodes: [{ id: 'h1', type: 'html', props: { source } }],
  })

  it('accepts a valid html node with inline script (iframe sandbox renders it)', () => {
    const doc = validateCanvasDocument(htmlDoc('<div style="padding:20px"><h1>Landing</h1><p>hero copy</p></div>'))
    expect(doc.nodes[0]?.type).toBe('html')
  })

  it('accepts html source up to 100KB, exempt from 16KB node cap', () => {
    const big = `<div>${'x'.repeat(60 * 1024)}</div>` // ~60KB，超过通用 16KB 上限
    expect(() => validateCanvasDocument(htmlDoc(big))).not.toThrow()
  })

  it('rejects html source over 100KB with actionable message', () => {
    const tooBig = `<div>${'x'.repeat(101 * 1024)}</div>`
    try {
      validateCanvasDocument(htmlDoc(tooBig))
      expect.unreachable()
    } catch (e) {
      expect((e as Error).message).toMatch(/exceeds max/)
      expect((e as Error).message).toContain('props.source')
    }
  })

  it('rejects html node missing source', () => {
    const bad = { title: 't', layout: 'grid', nodes: [{ id: 'h1', type: 'html', props: { title: 'x' } }] }
    expect(() => validateCanvasDocument(bad)).toThrow(/source.*is required/)
  })

  it('rejects non-string html source', () => {
    expect(() => validateCanvasDocument(htmlDoc(42 as unknown as string))).toThrow(/must be a string/)
  })
})
