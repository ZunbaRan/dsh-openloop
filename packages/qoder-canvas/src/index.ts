...
async function designSkillHint(ctx: Context, canvasId: string): Promise<string | null> {
  ...
  return `Canvas ${canvasId} created. TIP — design skills detected: ${list}. For rich HTML designs (their specialty), render them via the html_artifact tool in that skill's style; canvas DSL nodes stay for structured data content.`
}
