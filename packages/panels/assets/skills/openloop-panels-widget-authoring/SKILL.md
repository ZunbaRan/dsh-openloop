...
规则：
- url **必须 `https://`**，禁环回/内网（SSRF 防护）；仅公网 https，禁止凭据（不要传 Authorization 头；凭据永不过 DSH）
...
预设速查之外的 kind（如 form 类输入组件）属于**平台能力缺口**，不是让你绕过的障碍：

1. 输出「缺口报告」给维护者：缺什么 / 证据（校验返回了什么）/ 建议的扩展项（新 kind + props 契约草案）
2. **然后停下等指示**——现有能力能做的部分照常完成，缺口部分如实标注未实现
3. **禁止的越界方案**：修改 vendor/node_modules 里的平台包、就地打补丁、提议重编译重启用户生产实例——预设族扩展只在 openloop 源仓库进行，走版本化发布纪律（check → bump → pack → 双 profile 安装 → 真机验收）

> 判据：**在平台上建应用 ≠ 改平台**。数据录入等表单需求，先用现有能力（data-table + api 数据绑定 + app_backend 写路径）完成能完成的，表单预设族的缺口如实报告。
