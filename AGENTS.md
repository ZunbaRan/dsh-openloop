...
27. **sandbox iframe 与宿主通信的四个坑（0.12 整体 iframe 化实战）**：① sandbox iframe 的 Origin 头是字面量 `null`——`new URL('null')` 抛错，端点校验须显式放行；② `<script type="module">` 在 opaque origin 下加载是 CORS 模式——静态资源端点须回 `Access-Control-Allow-Origin: *`；③ 宿主在 iframe load 事件发 init——app 模块尚未执行必丢——app 启动后主动发 hello，宿主收到重发 init（拉模式）；④ iframe 内 console 不冒泡主 frame——调试用 postMessage 诊断信回传（魔数区分）。
28. **tsdown 多 entry 场景 bundle 资源差异**：主 entry（index.js）走 neverBundle/externals（React external 由宿主提供），app entry（浏览器直跑 ESM）必须 `noExternal: [/^react/]` 显式打入——devDeps/peerDeps 的 React 默认 external，产物首行 `import "react"` 在浏览器必败。
29. **dsh 0.1.0-rc.7 凭证格式迁移**：`.credentials.yaml` 从嵌套（version/refs/records）变为扁平 POSIX 标识符→字符串 map，`version` 必须带引号——旧版升级后启动报 `the value for "refs" must be a string`。迁移：备份后平铺 refs 键值；验证实例无凭证模式可跑（empty document = empty store）。
30. **沙箱 iframe 内 open shadow 的元素选择（0.12 实战）**：`document.elementsFromPoint` 在 opaque origin 下**不穿透** open shadow root——html 节点内选择要显式遍历各 `shadowRoot.elementFromPoint` 自查；命中容器时 drillToDeepest 向下钻（DevTools 语义）。
31. **滚动容器内 absolute 高亮的坐标补偿**：高亮框挂 pin-layer（absolute，参照滚动容器 padding box），计算必须在视口坐标差之外加 `scrollLeft/scrollTop`——漏了则「未滚动正常、一滚就错位」。
32. **标注回显优先存元素引用而不是回查（0.12.7 用户点破的最大教训）**：hover 命中时 el 引用就在手上，确认/保存后的高亮应直接用该引用（isConnected 校验）——不要走 domPath/querySelector/indexPath「重新查询」（CSS 选择器在 Tailwind class 含 `:` 下不可靠，且整条回查链都是多余复杂度）。回查只作为无引用时（持久化历史注释）的兜底。DOM 元素引用进 React state 安全，但**持久化（JSON.stringify）前必须剥离**（循环引用会炸）。
